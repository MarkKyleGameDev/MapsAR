...
namespace Mapbox.MapboxSdkCs.UnitTest
{
	...
	internal class TileJSONTest
	{
		...
		)
		{
			...
			Assert.AreEqual(boundsSouth, response.BoundsParsed.South, 0.052, "Bounds.South does not match");
			Assert.AreEqual(boundsEast, response.BoundsParsed.East, "Bounds.East does not match");
			Assert.AreEqual(boundsNorth, response.BoundsParsed.North, 0.052, "Bounds.North does not match");
